...
let view='adt',mode='agent',page='dashboard',agent='contractor',adtSidebarCollapsed=false,agentSidebarCollapsed=true,notifOpen=false,notifShowUnread=true,agentMsgs=[{role:'bot',text:"Hi John! I'm your ADT Agent. What would you like to do today?"}],formStep=-1,returnToReview=false,cw=360,selectedAIJourneyId='contract-to-payroll',aiEventDrawerIdx=-1;
let aiContractPrefill=null,aiAssistedFlow=false,aiCtNotFoundOpen=false,aiProposalDraft=null,aiCtChatMsgs=[];
const aiDealManager={name:'Karan Mehta',role:'Deal Manager',initials:'KM'};
let selectedAIRunId='RUN-1001';
const aiAutomationRuns={
  'contract-to-payroll':[
    {runId:'RUN-1001',client:'ABC Manufacturing GmbH',country:'Germany',contractType:'EOR',currentStepIdx:5,status:'Waiting for Approval',lastActivity:'2 hours ago'},
    {runId:'RUN-1002',client:'Bright Path Logistics B.V.',country:'Netherlands',contractType:'EOR',currentStepIdx:8,status:'Active',lastActivity:'5 hours ago'},
    {runId:'RUN-1003',client:'Sunrise Retail Ltd',country:'United Kingdom',contractType:'PEO',currentStepIdx:13,status:'Waiting for Approval',lastActivity:'1 day ago'},
    {runId:'RUN-1004',client:'Vertex Software S.L.',country:'Spain',contractType:'EOR',currentStepIdx:12,status:'Exception',lastActivity:'3 hours ago',exceptionNote:'Bank/payment details are missing for the payroll readiness check.'},
    {runId:'RUN-1005',client:'Meridian Consulting Pvt Ltd',country:'India',contractType:'Contractor',currentStepIdx:13,status:'Completed',lastActivity:'2 days ago'}
  ]
};
...
function addListingItem(pg){if(pg==='contracts'){const j=aiJourneys.find(x=>x.id==='contract-to-payroll');aiAssistedFlow=false;aiContractPrefill=null;page=(j&&j.status==='Active')?'ai-contract-assistant':'contract-type-select';renderADTPage();}else if(pg==='teams'){page='team-add';renderADTPage();}else if(pg==='all-leaves'){page='leave-add';renderADTPage();}}
...
// -- AI EXECUTIVE MODULE --
const aiJourneys=[
  {id:'contract-to-payroll',name:'Contract Generation to Ready for Payroll',desc:'Automates the flow from approved proposal, contract generation, client signing, onboarding checks, compliance validation, and payroll readiness.',modules:['Contracts','Proposal','Docuseal','Onboarding','Compliance','Payroll'],coverage:70,humanSteps:5,aiSteps:9,status:'Active',risk:'Medium',updated:'02 Jul 2026, 10:20 AM'},
  {id:'proposal-to-contract',name:'Proposal Approval to Contract Creation',desc:'Automates proposal data validation, pricing checks, internal approval routing, and client acceptance tracking through to contract creation.',modules:['Proposal','Compliance Hub','Rates & Rules','Contracts'],coverage:60,humanSteps:4,aiSteps:5,status:'Available',risk:'Low',updated:'28 Jun 2026, 4:10 PM'},
  {id:'onboarding-to-payroll',name:'Employee Onboarding to Payroll Readiness',desc:'Automates document collection tracking, compliance checks, compensation mapping, and payroll readiness scoring for new hires.',modules:['Onboarding','Compliance Hub','Payroll','Employee Profile'],coverage:65,humanSteps:5,aiSteps:6,status:'Available',risk:'Medium',updated:'25 Jun 2026, 11:45 AM'},
  {id:'payroll-validation',name:'Payroll Validation and Approval',desc:'Automates timesheet, leave, variable pay and statutory deduction calculations, flags exceptions, and routes for finance approval.',modules:['Payroll','Timesheet','Payheads','Finance'],coverage:75,humanSteps:3,aiSteps:7,status:'Available',risk:'High',updated:'20 Jun 2026, 9:05 AM'},
  {id:'compliance-doc-collection',name:'Compliance Document Collection',desc:'Automates compliance requirement identification, checklist generation, document validation, and expiry tracking with exception handling.',modules:['Compliance Hub','Onboarding','Rates & Rules'],coverage:68,humanSteps:2,aiSteps:7,status:'Draft',risk:'Low',updated:'15 Jun 2026, 2:30 PM'},
  {id:'invoice-reconciliation',name:'Invoice and Payment Reconciliation',desc:'Automates invoice generation, validation, payment matching, and discrepancy detection before finance approval and ledger updates.',modules:['Payments','Finance','Contracts'],coverage:55,humanSteps:3,aiSteps:6,status:'Draft',risk:'Medium',updated:'10 Jun 2026, 5:50 PM'}
];

const aiJourneyEvents={
  'contract-to-payroll':[
    {name:'Deal / Contract Process Created',chips:['System Action','Human Required'],source:'ADT Sales Process / All Contracts',desc:'Contract journey starts once a deal or contract process is created.',validation:'None — this is the trigger event.',human:'Required only for initial creation.',failure:'Journey cannot start until a deal or contract process exists.',next:'Proposal Data Fetched'},
    {name:'Proposal Data Fetched',chips:['AI Automated','Validation Required'],source:'Proposal module',desc:'AI fetches approved proposal data including client details, country, employment type, commercial terms, and proposal status.',validation:'Proposal must be approved.',human:'Not required — fully automated.',failure:'If proposal is not approved, AI pauses and notifies the deal owner.',next:'Client and Entity Mapping Validated',fields:['Client Details','Country','Employment Type','Commercial Terms','Proposal Status']},
    {name:'Client and Entity Mapping Validated',chips:['AI Automated','Validation Required','Exception Possible'],source:'Client master, Entity master, Country master',desc:'AI validates client entity, ADT legal entity, country, and employment type mapping.',validation:'Country and entity must match contract rules.',human:'Required only if a mismatch is found.',failure:'Mismatch is flagged to Admin for manual mapping correction.',next:'Contract Template Selected'},
    {name:'Contract Template Selected',chips:['AI Automated','Validation Required','Exception Possible'],source:'Contract Template module',desc:'AI selects the correct contract template based on country, employment type, and entity.',validation:'Active template must exist.',human:'Required only if no template is found.',failure:'Journey pauses and creates a task to add or activate a template.',next:'Contract Field Mapping Completed'},
    {name:'Contract Field Mapping Completed',chips:['AI Automated','Validation Required','Exception Possible'],source:'Proposal, Client Profile, Entity Master, Rates & Rules',desc:'AI maps required contract fields such as client name, entity name, country, payment terms, fee, security deposit, and signatory details.',validation:'All mandatory fields must be available. Country must match selected template. Employment type must match proposal. Rates & Rules must be active.',human:'Required when a mandatory field is missing, a template mismatch is detected, or the commercial value differs from the approved proposal.',failure:'Missing fields are highlighted for Admin before generation can proceed.',next:'Admin Review of Contract Data',fields:['Client Name','Client Entity','Country','Employment Type','Monthly Invoice Amount','Security Deposit','Payment Terms','ADT Entity','Signatory Name']},
    {name:'Admin Review of Contract Data',chips:['Human Required','Approval Required'],source:'Contract creation form',desc:'OpenDHI Admin reviews the pre-filled contract data before generation. AI highlights missing or risky fields.',validation:'Reviewer must approve, edit, or send back the data.',human:'Required — Admin must approve, edit, or send for correction.',failure:'If sent for correction, the journey routes back to field mapping.',next:'Contract Generated'},
    {name:'Contract Generated',chips:['AI Automated','System Action'],source:'Contract template engine',desc:'System generates the contract document using mapped fields and approved data.',validation:'Contract data must be approved.',human:'Not required — automatic generation.',failure:'Generation retries once; a persistent failure notifies Admin.',next:'Contract Sent to Client'},
    {name:'Contract Sent to Client',chips:['Human Required','Approval Required'],source:'Contract module',desc:'Admin confirms and sends the generated contract to the client. AI suggests email text and checks that all attachments are present.',validation:'All required attachments must be present.',human:'Required — Admin confirms and sends.',failure:'If attachments are missing, AI blocks the send and flags Admin.',next:'Client Signature / Docuseal Status Tracked'},
    {name:'Client Signature / Docuseal Status Tracked',chips:['System Action','Client Action'],source:'Docuseal / E-sign integration',desc:'System tracks whether the contract is signed by employee, client, and ADT as per the signing sequence. AI sends a reminder suggestion if pending.',validation:'All parties in the signing sequence must sign.',human:'Client action required to sign.',failure:'If unsigned past SLA, AI recommends escalation to Admin.',next:'Contract Approval Completed'},
    {name:'Contract Approval Completed',chips:['System Action'],source:'Contract module',desc:'Once signing is completed, contract status changes to approved and AI moves the journey to onboarding readiness check.',validation:'All signatures must be captured.',human:'Not required.',failure:'N/A — automatic status transition.',next:'Onboarding Checklist Created'},
    {name:'Onboarding Checklist Created',chips:['AI Automated','Validation Required'],source:'Onboarding module',desc:'AI creates an onboarding checklist based on country, employment type, and contract type.',validation:'Required documents and compliance checks must be mapped for the country.',human:'Not required.',failure:'If no checklist template exists for the country, the journey pauses for Admin setup.',next:'Employee / Client Documents Validated'},
    {name:'Employee / Client Documents Validated',chips:['AI Automated','Human Required','Exception Possible','Approval Required'],source:'Onboarding documents, Compliance module',desc:'AI validates submitted documents and flags missing, expired, or incorrect documents.',validation:'All checklist documents must be present, current, and correctly formatted.',human:'Required only for exceptions — missing, expired, or incorrect documents.',failure:'Exception is routed to Admin with the specific document issue.',next:'Payroll Readiness Data Checked'},
    {name:'Payroll Readiness Data Checked',chips:['AI Automated','Validation Required','Exception Possible'],source:'Payroll module, Employee profile, Compensation structure, Bank details, Compliance rules',desc:'AI checks whether the employee or client has all required payroll inputs — compensation, salary structure, bank details, country payroll rules, compliance documents, and contract approval.',validation:'Compensation available. Salary structure configured. Bank/payment details available. Country payroll rules available. Required compliance documents available. Contract approved.',human:'Required only if data is missing.',failure:'Missing items are listed as blockers for Payroll Admin.',next:'Ready for Payroll Confirmation',fields:['Compensation Structure','Bank/Payment Details','Country Payroll Rules','Compliance Documents','Contract Approval Status']},
    {name:'Ready for Payroll Confirmation',chips:['Human Required','Approval Required'],source:'Payroll module',desc:'Payroll Admin confirms final readiness. AI shows a readiness score and pending blockers.',validation:'All payroll readiness checks must be cleared.',human:'Required — Payroll Admin confirms.',failure:'If blockers remain, Admin is shown the specific missing items.',next:'Journey Complete — Ready for Payroll'}
  ],
  'proposal-to-contract':[
    {name:'Proposal Drafted',chips:['Human Required'],source:'Proposal module',desc:'Sales team drafts the initial proposal with client, country, and commercial terms.',validation:'None — manual entry point.',human:'Required for initial drafting.',failure:'Journey cannot proceed without a drafted proposal.',next:'Proposal Data Validated'},
    {name:'Proposal Data Validated',chips:['AI Automated','Validation Required'],source:'Proposal module, Client master',desc:'AI validates client details, country, and employment type entered on the proposal.',validation:'Client and country must exist in master data.',human:'Not required — fully automated.',failure:'Invalid entries are flagged back to the drafting rep.',next:'Pricing & Commercial Terms Checked'},
    {name:'Pricing & Commercial Terms Checked',chips:['AI Automated','Validation Required','Exception Possible'],source:'Rates & Rules module',desc:'AI checks proposed fees and commercial terms against approved rate cards for the country.',validation:'Pricing must fall within approved rate card bands.',human:'Required only if pricing falls outside approved bands.',failure:'Out-of-band pricing is routed to a pricing exception review.',next:'Compliance Pre-Check'},
    {name:'Compliance Pre-Check',chips:['AI Automated','Validation Required'],source:'Compliance Hub',desc:'AI runs a preliminary compliance check for the country and employment type.',validation:'Country must support the proposed employment type.',human:'Not required — informational check.',failure:'Unsupported combinations are flagged before internal approval.',next:'Internal Approval Routing'},
    {name:'Internal Approval Routing',chips:['Human Required','Approval Required'],source:'Proposal module',desc:'Proposal is routed to the relevant internal approver based on deal size and country.',validation:'Approver must sign off before client review.',human:'Required — internal approval.',failure:'Rejected proposals route back to drafting.',next:'Client Review Sent'},
    {name:'Client Review Sent',chips:['Human Required'],source:'Proposal module',desc:'Approved proposal is sent to the client for review and acceptance.',validation:'Proposal must carry internal approval.',human:'Required — Admin sends the proposal.',failure:'N/A — manual send step.',next:'Client Acceptance Tracked'},
    {name:'Client Acceptance Tracked',chips:['System Action','Client Action'],source:'Proposal module',desc:'System tracks client acceptance status. AI sends a reminder suggestion if pending beyond SLA.',validation:'Client must accept or reject.',human:'Client action required.',failure:'If unaccepted past SLA, AI recommends escalation.',next:'Proposal Approved'},
    {name:'Proposal Approved',chips:['System Action'],source:'Proposal module',desc:'Proposal status changes to approved once client acceptance is recorded.',validation:'Client acceptance must be recorded.',human:'Not required.',failure:'N/A — automatic status transition.',next:'Contract Creation Triggered'},
    {name:'Contract Creation Triggered',chips:['AI Automated','System Action'],source:'Contracts module',desc:'AI automatically triggers the Contract Generation to Ready for Payroll journey using the approved proposal data.',validation:'Approved proposal must be available.',human:'Not required — fully automated handoff.',failure:'If the contract journey fails to trigger, Admin is notified to start it manually.',next:'Journey Complete — Contract Journey Triggered'}
  ],
  'onboarding-to-payroll':[
    {name:'Onboarding Initiated',chips:['System Action'],source:'Onboarding module',desc:'Onboarding record is created automatically once the contract is approved.',validation:'Contract must be approved.',human:'Not required.',failure:'N/A — automatic trigger.',next:'Employee Documents Requested'},
    {name:'Employee Documents Requested',chips:['AI Automated','System Action'],source:'Onboarding module',desc:'AI generates and sends the document checklist to the employee or client based on country and employment type.',validation:'Country document checklist must be mapped.',human:'Not required.',failure:'If no checklist mapping exists, Admin is notified to configure one.',next:'Document Collection Tracked'},
    {name:'Document Collection Tracked',chips:['System Action','Client Action'],source:'Onboarding documents',desc:'System tracks uploaded documents against the checklist. AI sends reminders for pending items.',validation:'All checklist items must be uploaded.',human:'Client/employee action required to upload.',failure:'Overdue items are escalated to Admin.',next:'Document Validation'},
    {name:'Document Validation',chips:['AI Automated','Human Required','Exception Possible'],source:'Onboarding documents, Compliance module',desc:'AI validates document authenticity, expiry, and completeness.',validation:'Documents must be current, complete, and correctly formatted.',human:'Required only for exceptions.',failure:'Exceptions are routed to Admin with the specific issue.',next:'Background / Compliance Check'},
    {name:'Background / Compliance Check',chips:['AI Automated','Validation Required'],source:'Compliance Hub',desc:'AI runs required background and compliance checks for the country.',validation:'All mandatory country compliance checks must pass.',human:'Not required unless a check fails.',failure:'Failed checks are routed to the Compliance Officer.',next:'Bank Details Captured'},
    {name:'Bank Details Captured',chips:['Human Required','Client Action'],source:'Employee profile',desc:'Employee or client submits bank and payment details required for payroll disbursement.',validation:'Bank details must pass format validation.',human:'Client/employee action required.',failure:'Invalid bank details are flagged back for correction.',next:'Compensation Structure Mapped'},
    {name:'Compensation Structure Mapped',chips:['AI Automated','Validation Required'],source:'Rates & Rules, Contract data',desc:'AI maps the approved compensation from the contract into the payroll compensation structure.',validation:'Compensation must match approved contract terms.',human:'Not required.',failure:'Mismatches are flagged for Payroll Admin review.',next:'Statutory Registrations Checked'},
    {name:'Statutory Registrations Checked',chips:['AI Automated','Validation Required','Exception Possible'],source:'Compliance Hub, Payroll module',desc:'AI checks whether required statutory registrations (tax ID, social security, etc.) exist for the employee in the target country.',validation:'Required statutory IDs must be present or in progress.',human:'Required only if a registration is missing.',failure:'Missing registrations are flagged as a blocker for Payroll Admin.',next:'Onboarding Admin Review'},
    {name:'Onboarding Admin Review',chips:['Human Required','Approval Required'],source:'Onboarding module',desc:'Onboarding Admin reviews the completed onboarding profile before payroll handoff.',validation:'All onboarding steps must be complete.',human:'Required — Admin approval.',failure:'If rejected, the journey routes back to the relevant step.',next:'Payroll Readiness Check'},
    {name:'Payroll Readiness Check',chips:['AI Automated','Validation Required'],source:'Payroll module',desc:'AI runs a final payroll readiness score across compensation, bank details, statutory IDs, and compliance documents.',validation:'All payroll prerequisites must be satisfied.',human:'Not required unless blockers remain.',failure:'Remaining blockers are listed for Payroll Admin.',next:'Ready for Payroll Confirmation'},
    {name:'Ready for Payroll Confirmation',chips:['Human Required','Approval Required'],source:'Payroll module',desc:'Payroll Admin gives final confirmation that the employee is ready for the next payroll cycle.',validation:'Readiness score must be 100%.',human:'Required — Payroll Admin confirms.',failure:'If blockers remain, confirmation is blocked.',next:'Journey Complete — Ready for Payroll'}
  ],
  'payroll-validation':[
    {name:'Payroll Cycle Initiated',chips:['System Action'],source:'Payroll module',desc:'Payroll cycle opens automatically on the scheduled cutoff date.',validation:'None — scheduled trigger.',human:'Not required.',failure:'N/A — automatic trigger.',next:'Attendance & Timesheet Data Fetched'},
    {name:'Attendance & Timesheet Data Fetched',chips:['AI Automated','Validation Required'],source:'Timesheet module',desc:'AI fetches approved timesheet and attendance data for the payroll cycle.',validation:'Timesheets must be submitted and approved for the cycle.',human:'Not required.',failure:'Missing timesheets are flagged for the employee\'s manager.',next:'Leave Deductions Calculated',fields:['Approved Hours','Overtime','Attendance Exceptions']},
    {name:'Leave Deductions Calculated',chips:['AI Automated'],source:'Leaves module',desc:'AI calculates unpaid leave deductions based on approved leave records for the cycle.',validation:'Leave records must be finalized for the cycle.',human:'Not required.',failure:'Unresolved leave requests are flagged before calculation completes.',next:'Variable Pay & Bonus Fetched'},
    {name:'Variable Pay & Bonus Fetched',chips:['AI Automated','Validation Required'],source:'Payheads module',desc:'AI fetches approved variable pay, incentives, and bonus components for the cycle.',validation:'Variable pay entries must be approved.',human:'Not required.',failure:'Unapproved entries are excluded and flagged for review.',next:'Statutory Deductions Calculated'},
    {name:'Statutory Deductions Calculated',chips:['AI Automated','Validation Required'],source:'Rates & Rules, Compliance Hub',desc:'AI calculates statutory deductions (tax, social security, etc.) using country-specific rates.',validation:'Country rate tables must be current.',human:'Not required.',failure:'Outdated rate tables block calculation and notify Compliance.',next:'Payroll Exceptions Flagged'},
    {name:'Payroll Exceptions Flagged',chips:['AI Automated','Exception Possible'],source:'Payroll module',desc:'AI scans the computed payroll run for anomalies such as large swings, negative net pay, or duplicate entries.',validation:'Computed values must fall within expected variance thresholds.',human:'Required only for flagged exceptions.',failure:'Flagged records are held out of the batch pending review.',next:'Payroll Admin Review'},
    {name:'Payroll Admin Review',chips:['Human Required','Approval Required'],source:'Payroll module',desc:'Payroll Admin reviews the computed payroll run and resolves flagged exceptions.',validation:'All exceptions must be resolved or accepted.',human:'Required — Admin review.',failure:'Unresolved exceptions block progression to Finance approval.',next:'Finance Approval'},
    {name:'Finance Approval',chips:['Human Required','Approval Required'],source:'Finance module',desc:'Finance approves the payroll run for disbursement.',validation:'Payroll run must be reviewed and free of open exceptions.',human:'Required — Finance approval.',failure:'Rejected runs route back to Payroll Admin.',next:'Payslips Generated'},
    {name:'Payslips Generated',chips:['AI Automated','System Action'],source:'Payroll module',desc:'System generates payslips for all employees in the approved run.',validation:'Payroll run must be Finance-approved.',human:'Not required.',failure:'Generation retries once; persistent failure notifies Payroll Admin.',next:'Payroll Disbursed'},
    {name:'Payroll Disbursed',chips:['Human Required','System Action'],source:'Finance / Banking integration',desc:'Payroll Admin initiates disbursement; system executes the payment batch.',validation:'Bank file must pass validation before submission.',human:'Required — Admin initiates disbursement.',failure:'Failed disbursements are flagged for manual retry.',next:'Journey Complete — Payroll Disbursed'}
  ],
  'compliance-doc-collection':[
    {name:'Compliance Requirement Identified',chips:['AI Automated'],source:'Compliance Hub',desc:'AI identifies applicable compliance requirements based on country and employment type.',validation:'Country compliance rule set must exist.',human:'Not required.',failure:'Missing rule sets notify the Compliance Officer.',next:'Document Checklist Generated'},
    {name:'Document Checklist Generated',chips:['AI Automated'],source:'Compliance Hub',desc:'AI generates the required document checklist for the employee or client.',validation:'Checklist template must be mapped for the country.',human:'Not required.',failure:'Missing template pauses the journey for Admin setup.',next:'Client/Employee Notified'},
    {name:'Client/Employee Notified',chips:['System Action'],source:'Compliance Hub',desc:'System notifies the client or employee with the document checklist and submission link.',validation:'Valid contact details must be on file.',human:'Not required.',failure:'Missing contact details are flagged for Admin.',next:'Documents Uploaded'},
    {name:'Documents Uploaded',chips:['Client Action'],source:'Compliance Hub',desc:'Client or employee uploads the requested compliance documents.',validation:'File formats must meet upload requirements.',human:'Client/employee action required.',failure:'Overdue uploads are escalated with a reminder.',next:'Document Validation'},
    {name:'Document Validation',chips:['AI Automated','Validation Required'],source:'Compliance Hub',desc:'AI validates document authenticity, format, and completeness against the checklist.',validation:'Documents must match checklist requirements.',human:'Not required unless validation fails.',failure:'Invalid documents are sent back with the specific reason.',next:'Expiry/Version Check'},
    {name:'Expiry/Version Check',chips:['AI Automated','Exception Possible'],source:'Compliance Hub',desc:'AI checks document expiry dates and version currency against country requirements.',validation:'Documents must not be expired or superseded.',human:'Required only for exceptions.',failure:'Expired documents trigger a re-request to the client.',next:'Exception Flagged for Missing Docs'},
    {name:'Exception Flagged for Missing Docs',chips:['AI Automated','Human Required','Approval Required'],source:'Compliance Hub',desc:'AI flags any checklist items still missing or invalid for Compliance Officer attention.',validation:'All checklist items must eventually resolve to valid or explicitly waived.',human:'Required for exception review and waiver decisions.',failure:'Unresolved exceptions keep the compliance record open.',next:'Compliance Officer Review'},
    {name:'Compliance Officer Review',chips:['Human Required','Approval Required'],source:'Compliance Hub',desc:'Compliance Officer reviews the full document set and approves or rejects compliance status.',validation:'All required documents must be valid or waived.',human:'Required — Officer approval.',failure:'Rejected records route back to document collection.',next:'Compliance Status Updated'},
    {name:'Compliance Status Updated',chips:['System Action'],source:'Compliance Hub',desc:'System updates the compliance status, unblocking downstream onboarding and payroll steps.',validation:'Officer approval must be recorded.',human:'Not required.',failure:'N/A — automatic status update.',next:'Journey Complete — Compliance Cleared'}
  ],
  'invoice-reconciliation':[
    {name:'Invoice Generated',chips:['AI Automated','System Action'],source:'Payments module, Contracts',desc:'AI generates the invoice using approved contract terms and billing cycle data.',validation:'Contract must be active and billing terms available.',human:'Not required.',failure:'Missing billing terms pause generation and notify Finance.',next:'Invoice Data Validated'},
    {name:'Invoice Data Validated',chips:['AI Automated','Validation Required'],source:'Payments module',desc:'AI validates invoice line items, amounts, and tax calculations against contract and country rules.',validation:'Line items must match contract terms and applicable tax rules.',human:'Not required unless a mismatch is found.',failure:'Mismatches are flagged for Finance review before sending.',next:'Invoice Sent to Client'},
    {name:'Invoice Sent to Client',chips:['Human Required'],source:'Payments module',desc:'Finance confirms and sends the validated invoice to the client.',validation:'Invoice must pass data validation.',human:'Required — Finance sends the invoice.',failure:'N/A — manual send step.',next:'Payment Received Tracked'},
    {name:'Payment Received Tracked',chips:['System Action'],source:'Banking / Payments integration',desc:'System tracks incoming payments against outstanding invoices.',validation:'Payment must be received via a linked banking channel.',human:'Not required.',failure:'No incoming payment by due date triggers a reminder workflow.',next:'Payment Matched to Invoice'},
    {name:'Payment Matched to Invoice',chips:['AI Automated','Validation Required'],source:'Payments module',desc:'AI matches incoming payments to the correct invoice based on amount, reference, and client.',validation:'Payment amount and reference must correspond to an open invoice.',human:'Not required unless no confident match is found.',failure:'Unmatched payments are queued for manual matching.',next:'Discrepancy Detected'},
    {name:'Discrepancy Detected',chips:['AI Automated','Exception Possible'],source:'Payments module',desc:'AI flags discrepancies such as partial payments, overpayments, or currency mismatches.',validation:'Matched payment amount must equal invoice amount.',human:'Required only when a discrepancy is detected.',failure:'Discrepant records are held out of auto-reconciliation.',next:'Finance Review of Discrepancy'},
    {name:'Finance Review of Discrepancy',chips:['Human Required','Approval Required'],source:'Finance module',desc:'Finance reviews flagged discrepancies and decides on adjustment, write-off, or client follow-up.',validation:'A resolution decision must be recorded.',human:'Required — Finance review.',failure:'Unresolved discrepancies remain open on the reconciliation queue.',next:'Reconciliation Approved'},
    {name:'Reconciliation Approved',chips:['Human Required','Approval Required'],source:'Finance module',desc:'Finance gives final approval to close out the reconciliation for the invoice.',validation:'All discrepancies must be resolved or explicitly accepted.',human:'Required — Finance approval.',failure:'N/A — final approval step.',next:'Ledger Updated'},
    {name:'Ledger Updated',chips:['System Action'],source:'Finance module',desc:'System posts the reconciled entry to the ledger and closes the invoice.',validation:'Reconciliation must be approved.',human:'Not required.',failure:'N/A — automatic ledger posting.',next:'Journey Complete — Reconciled'}
  ]
};
const allLeavesData=[
